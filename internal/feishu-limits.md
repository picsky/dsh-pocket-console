...
| **输入框长度** | **≤1000 字符**，超出客户端报错 | **没设 `max_length`，也没在文案里说明**；回复框与会话框都是**单行 `text`**（未用 `multiline_text`） | ❌ 真机已撞到 |
...
| L4 | `max_length` 超限时客户端**报错还是静默截断** | 决定要不要在文案里提示"最多 1000 字" | 真机输 1001 字符 |
...
| 容器嵌套深度 | **≤5**（硬性） | 官方各容器文档 |
| 输入框可收字数 | **按 1000 规划**，超出要走别的路（多段提交 / 提示用户） | 官方 [1,1000] 硬顶 |
