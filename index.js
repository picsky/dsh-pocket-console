 ...
 * writing the marker costs the whole form on a Host whose library lacks it, which
 * is any profile that hoists a schemastery older than the one the Host carries —
 * the peer range takes whatever resolves, and 3.18.1 beside a 0.1.7 Host leaves
 * every field unmarked, with the card able to report only that the Host serves no
 * form. So `extra` is the floor, and the helper is used when it is there.
