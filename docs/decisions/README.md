...
| [0032](0032-a-stand-in-is-held-to-the-host.md) | A stand-in is held to the Host's own contract, the contract lives in the repository as executable code, and a case asserts the user-visible exit |
...
| [0035](0035-the-instruction-box-says-what-it-holds.md) | The instruction box is a multi-line box that states the platform's 1000-character limit, written down in the code and in the copy rather than inherited from a default |
| [0036](0036-a-quoted-card-is-answered-by-the-message.md) | A quoted card that is waiting for an answer is answered by the message: the text becomes the payload the card's own button would have sent, an approval answers to two words and to nothing else, and no word can switch a session's policy |
