...
其中三项是**设置命名空间**：可以在设置卡片里改（设置 → 插件 → 插件配置 →「口袋控制台」），
...
它刻意是**两个开关**而不是一个：这个控制**插件说什么**，部署的日志级别控制**什么能到你的终端**
——需要「插件 `debug: on`」**加上**「日志本身按 `debug` 级别运行」。
一个能让宿主的日志比宿主自己要求的更吵的插件，等于插件在替宿主决定噪音量。
