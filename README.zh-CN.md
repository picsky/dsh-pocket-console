...
- 通知一旦不再是该会话的最新消息就停止接受回复：被更新的结果取代，或者会话有了新输入（来自任何界面）——包括 **dsh 没在运行时**发生的输入，这一点是去会话自己的日志里核对的。**没有时间限制**——你第二天回来回复它，它仍然有效，**重启 dsh 也不会把它收回**。
  卡片会被改写成对应说明，所以回复永远不会把"针对旧结果的指令"塞进已经走远的会话。回复需要会话正在运行：会话重新打开之前，卡片会说明该会话已不在运行，这与"结果过期"是两回事。
