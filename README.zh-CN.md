...
- **手机丢了，损害的上界是一次回答。** 卡片本身就是凭证，只有绑定的接收人能按：私聊只绑定尚未绑定的部署，群消息永不绑定。
...
- **不搬 GUI。** 搬界面就要搬工作区、会话、设置和凭据；这里只搬一个决定——所以你也**没法在手机上接着干活**。
- **默认不主动推手机。** 只有桌面没在时限内应答时，卡片才会发出。
...
- **卡片文本按字节限额，不是字符数。** 请求体上限 30 KB、一个汉字 3 字节，所以文本限制在 9 KB 以内并在截断时说明；超长的计划以开头部分送达，按钮照常可用。
