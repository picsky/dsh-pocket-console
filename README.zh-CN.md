...
- 通知会在三种情况下停止接受回复：被更新的结果取代、超过 `resultNoticeTtlSeconds`、或者会话有了新输入（来自任何界面）——卡片会被改写成对应说明。
  否则对一条过时通知的回复，会把针对旧结果的指令塞进已经走远的会话。
...
31 个用例覆盖：设置命名空间与路由注册、未绑定时不升级、unbound → awaiting → bound 状态机、
...
通知在被取代/会话有新输入/超出有效期时拒绝回复、重启后凭据直连（不触发扫码）、
