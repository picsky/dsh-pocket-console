...
const zh = {
  ...
  answerPlaceholder: '输入回答（最多 1000 字）',
  notePlaceholder: '补充说明（可选，最多 1000 字）',
  ...
  workOfferHint: '---\n\n或者，在**新会话**里开下一段：',
  workPlaceholder: '新会话要做什么？（最多 1000 字）',
  workStart: '开始新任务',
  ...
  fullAccessConfirmText: '这个会话不再受沙箱限制，也不会再弹审批——**这一路对手机就静默了**。仍然需要审批的动作会被**直接拒绝**（不是自动放行）。要改回来得到桌面设置里。',
  fullAccessSettled: label => `已切到**${label}**（这一条也算批准了）：这个会话的沙箱不再拦截，也不会再弹审批，手机上不会再看到它的审批卡。仍然需要审批的动作会被**直接拒绝**。要改回来：桌面设置里选回「工作区内修改」。`,
  fullAccessOn: label => `已切到${label}`,
  fullAccessUnavailable: '这台宿主没有提供可切换的完全权限预设，这次只能按「允许一次」；审批卡没有变化。',
  fullAccessFailed: '没能切到完全权限（原因在部署日志里）。这次审批还没有决定，可以照常按「允许一次」或「拒绝」。',
  logFullAccessFailed: '读写权限预设时出错。',
  ...
  logSettingsTransportUnknown: capabilities => `本宿主的设置服务既没有 installSection 也没有 configure（探测到：${capabilities}），所以卡片里的设置项不会被宿主保存，需要改配置请编辑 YAML。`,
...
const en = {
  ...
  answerPlaceholder: 'Type your answer (up to 1000 characters)',
  notePlaceholder: 'Extra note (optional, up to 1000 characters)',
  ...
  workPlaceholder: 'What should the new session do? (up to 1000 characters)',
  ...
  fullAccessSettled: label => `Switched to **${label}** (this request was granted too): this session is no longer confined by the sandbox and will not ask for approval again, so no more approval cards will reach the phone. Actions that still need approval are **refused**. To change it back: pick the workspace-write preset in the desk settings.`,
  fullAccessOn: label => `switched to ${label}`,
  fullAccessUnavailable: 'this host offers no full-access preset to switch to, so this one can only be allowed once; the card is unchanged.',
  fullAccessFailed: 'the switch to full access did not happen (the deployment log has the reason). This approval is still undecided, and Allow once or Reject work as usual.',
  ...
  logSettingsTransportUnknown: capabilities => `this Host's settings service offers neither installSection nor configure (probed: ${capabilities}), so edits made on the card will not be persisted by the Host; change settings in YAML instead.`,
